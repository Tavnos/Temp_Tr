...
class Cl_Ca:
    ...
    def graph(self, xlim_a, xlim_b, function, tangent=None, ylim=None):
        ...
        plt.close()
    def param(self, var_a, var_b, var_c):
        self.fig, self.ax = plt.subplots()
        self.ax.set_xlim((-3,3))
        self.ax.set_ylim((-3,3))
        self.line, = self.ax.plot([],[],lw=2)
        self.t_var = np.linspace(-2*np.pi,2*np.pi,1000)
        self.a_var = var_a
        self.b_var = var_b
        self.c_var = var_c
    def init(self):
    ...
    def animate(self, i_var):
        func_var_a = np.e**(2*np.pi*1j*pot[self.a_var]*(self.t_var+i_var*.1)) 
        func_var_b = np.e**(2*np.pi*1j*pot[self.b_var]*(self.t_var+i_var*.1)) 
        func_var_c = np.e**(2*np.pi*1j*pot[self.c_var]*(self.t_var+i_var*.1)) 
