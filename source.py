...
async def on_message(message):
    if message.content.startswith('!help'):
        ...
        channel = message.channel
        msg = 'I have been running for {} seconds'.format(c_clc.get_uptime(new_upt))
        await channel.send(msg)
    ...
    elif message.content.startswith('!animplot'): 
        channel = message.channel
        new_str = message.content
        new_str = new_str.replace('!animplot ','')
        new_ls = new_str.split()
        c_clc.param(int(new_ls[0]), int(new_ls[1]), int(new_ls[2]))
        c_clc.anim_def()
        c_clc.g_render()
        await channel.send(file = discord.File('Science_Discord_Bot/plotimg/test_a.gif'))
